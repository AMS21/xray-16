...
IC void CSSafeMapIterator::add(const _key_type& id, _data_type* value, bool no_assert)
{
    ...
    if (I != m_objects.end())
    {
        //THROW2(no_assert, "Specified object has been already found in the registry!"); // CoC hack
        return;
    }
...
IC void CSSafeMapIterator::remove(const _key_type& id, bool no_assert)
{
    ...
    if (I == m_objects.end())
    {
        //THROW2(no_assert, "Specified object hasn't been found in the registry!"); // CoC hack
        return;
    }
